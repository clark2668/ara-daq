...
#define THIS_STATION ARA_STATION1B

//! Part of AraEvent library. This will be the beginnings of what the trigger information means



enum {
  softTrigger_FORCED=0, ///< Software trigger for normal running
  softTrigger_PED=1     ///< Software trigger for pedestal mode

} ;
typedef uint8_t AraAtriSoftTriggerInfo_t;  ///< Ensure that it is just 8 bits
...
typedef struct {
  ...
  uint32_t triggerInfo[MAX_TRIG_BLOCKS]; ///< The trigger information - one per trigger type
