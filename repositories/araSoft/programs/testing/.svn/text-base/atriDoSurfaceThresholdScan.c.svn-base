...
int atriSurfaceSetThresholds(unsigned int fd,
		      AtriDaughterStack_t stack,
  ...
  if (atriI2CWrite(fd, stack, surfaceDacAddress, 12, i2cDacWrite,
...
int main(int argc, char **argv)
{
  ...
  // First program the daughter stack.
  setAtriL1HighDb(fAtriControlSockFd, stack);
  ...
  for( j=low_threshold ; j<=high_threshold ; j=j+step_size ) {
     ...
     // - set the thresholds
     setSurfaceAtriThresholds( fAtriControlSockFd , stack , thresholds );
     printf("%d ",stack_int);
     printf("%f %5d ",
     ...
     // Surface scalers are at 0x120, always.
     controlPacket.data[1]=0x01;                               /* 0x10 - high order byte of address for TDA scaler registers is 0x0100          */
     controlPacket.data[2]=0x20;                         /* 0x00 - low order byte of address for TDA scaler registers for a daughter card */
