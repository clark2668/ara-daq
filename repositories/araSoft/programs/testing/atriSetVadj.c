/*! \file atriSetVadj.c
  PSA: silly testing, set Vadj value on DDA 
*/
...
int atriSetVadj(unsigned int fd,
		AtriDaughterStack_t stack,
		unsigned short Vadj,
		AtriControlPacket_t *controlPacket,
		AtriControlPacket_t *responsePacket) {
  ...
  // DAC command byte
  i2cDacWrite[0] = ddaVadj | dacWriteUpdateOne;
  i2cDacWrite[1] = (Vadj & 0xFF00)>>8;
  i2cDacWrite[2] = (Vadj & 0x00FF);

  if (atriI2CWrite(fd, stack, ddaDacAddress, 3, i2cDacWrite,
		   controlPacket, responsePacket) < 0) {
    fprintf(stderr, "%s: error writing to DDA Vadj DAC\n",
	    __FUNCTION__);
    return -1;
  }
  return 0;
...
int main(int argc, char **argv)
{
  ...
  // Prepare 
  if (argc != 3) {
    fprintf(stderr, "atriSetVadj: need 2 arguments: daughter stack number, and Vadj value\n");
    exit(1);
  }
  stack_int = strtoul(argv[1],0, NULL);
  stack = stack_int - 1;
  if (!stack_int || stack > D4) {
    fprintf(stderr, "atriSetVadj: stack number out of bounds (1 to 4)\n");
    exit(1);
  }
  Vadj = strtoul(argv[2], 0, NULL);
  int fAtriControlSockFd=openAtriControlSocket();
  if(!fAtriControlSockFd) {
    fprintf(stderr, "%s : could not open atri_control socket!\n", argv[0]);
    exit(1);
  }
  atriSetVadj(fAtriControlSockFd, stack, Vadj, &controlPacket, &responsePacket);
  printf("%s Vadj: %f (%4.4x)\n", atriDaughterStackStrings[stack],
	 dacToVolts(Vadj), Vadj);
