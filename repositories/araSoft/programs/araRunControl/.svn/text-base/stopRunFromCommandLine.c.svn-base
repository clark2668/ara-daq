/*! \file stopRunFromCommandLine.c
  \brief The stopRunFromCommandLine program is the standalone program for ending a run
         in which the run information is input via command line arguments 1 and 2 instead
         of by hand or from a file.  The program should be run via the following command:

                      stopNewRunFromCommandLine "User Name" "Run Comment"

         where the quotes are necessary.

    
  This is the updated stopRun 
...
int main(int argc, char *argv[])
{
  ...
  printf("\n\nYou can use the up arrow key to retrieve previous input to the logger (if you don't want to retype your name, etc.\n\n");
    ...
    usersName=argv[1]; // readline("Enter User's name:\n");
    printf("Enter User's name: [from argument 1]\n%s\n",usersName);
    if(usersName && *usersName)
        add_history(usersName);
    runDescription=argv[2]; // readline("Enter Reason for stopping  (ends with Enter):\n");
    printf("Enter Reason for stopping  (ends with Enter): [from argument 2]\n%s\n",runDescription);
