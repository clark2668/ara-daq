/*! \file startNewRunFromCommandLine.c
  \brief The startNewRunFromCommandLine program is the standalone program for starting a new
         run in which the run information is input via command line arguments 1 and 2 instead
         of by hand or from a file.  The program should be run via the following command:

                         startNewRunFromCommandLine "User Name" "Run Comment"
...
int main(int argc, char *argv[])
{
  ...
  runDescription=argv[2]; // readline("Enter Run Description (ends with Enter):\n");
  printf("Enter Run Description (ends with Enter): [from argument 2]\n%s\n",runDescription);
