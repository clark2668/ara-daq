...
/********************************************************************
*
* kvpGetUnsignedIntArray - Get value associated with key as array of integers
*
* <Insert longer description here>
*
* RETURNS:  
*
*/
KvpErrorCode kvpGetUnsignedShortArray (
				     const char* key,
				     ...
				     )
{
   ...
   if (index >= 0) {
      if (valTypes[index] == KT_INT) {
         ...
         for (element = 0 ; element < *entries ; element++) {
	   unsigned int fred =strtoul (valString, &endPtr, 0) ;
	   if(fred<65535) 
	     values[element] = fred;
	   else values[element]=65535;
            if (valString == endPtr) {
               lastError = KVP_E_BADVALUE ;
               lastError = KVP_E_BADVALUE ;
               break ;
...
{
  ARA_LOG_MESSAGE(LOG_DEBUG, "kvp error -- %s\n", kvpErrorString (code)) ;
}
