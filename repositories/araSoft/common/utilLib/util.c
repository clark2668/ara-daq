...
int newWriterFile(ARAWriterStruct_t* writer){
  ...
  if(writer->fileCount >= writer->maxFiles ){
    ...
    if( retVal ){
      ARA_LOG_MESSAGE(LOG_ERR,"Failed to make new sub directory needed for new file");
      return retVal;
    }
    writer->fileCount = 0;
  }
...
//ARA_LOG_MESSAGE(LOG_INFO,"Hello 3 -- %s",writer->filePrefix);
  ...
  if( !writer->currentFilePtr ){
    ARA_LOG_MESSAGE(LOG_ERR,"Failed to open file %s:\t%s",writer->currentFileName,strerror(errno));
    return errno;
  }
  writer->fileCount++;
...
//  ARA_LOG_MESSAGE(LOG_INFO,"writeBuffer() - new output file started");
  ...
  ARA_LOG_MESSAGE(LOG_DEBUG,"%s: gzwrite of %dB to file pointer %d (return %d)\n", __FUNCTION__,
		  len, writer->currentFilePtr, retVal);
  if(retVal<=0){
    int error;
    const char* errStr = gzerror(writer->currentFilePtr,&error);
    ARA_LOG_MESSAGE(LOG_ERR,"Error writing to file %s;\t%s (%d)",
  ...
  if(retVal<len)
    ARA_LOG_MESSAGE(LOG_WARNING,"Incomplete buffer %d in file %s; %d of %d bytes written",
...
int removeFile(const char *theFile)
{
  ...
  if(retVal!=0) {
    if(errorCounter<100) {
      ARA_LOG_MESSAGE(LOG_ERR,"Error (%d of 100) removing %s:\t%s",errorCounter,theFile,strerror(errno));
      errorCounter++;
    }
  }
  return retVal;
...
int makeDirectories(const char *theTmpDir) 
{
  ...
  while(subDir != NULL) {
	...
	if(errorCounter<100){
	  errorCounter++;
	  ARA_LOG_MESSAGE(LOG_ERR,"Error (%d of 100) making directory %s:\t%s",errorCounter,newDir,strerror(errno));
	}
	break;
      }
    }
...
int makeLink(const char *theFile, const char *theLinkDir)
{
  ...
  if(retVal!=0) {
    if(errorCounter<100) {
      errorCounter++;
      ARA_LOG_MESSAGE(LOG_ERR,"Error (%d of 100) linking %s to %s:\t%s",errorCounter,theFile,newFile,strerror(errno));
    }
  }
  return retVal;
}
...
int moveFile(const char *theFile, const char *theDir)
{
  ...
  if(!retVal && errorCounter<100){
    errorCounter++;
    ARA_LOG_MESSAGE(LOG_ERR,"Error (%d of 100) moving file %s to %s:\t%s",errorCounter,theFile,newFile,strerror(errno));
  }
  return retVal;    
}
...
int checkPidFile(const char *fileName)
{
  ...
  if (!(fpPid=fopen(fileName, "r"))) {
	...
	ARA_LOG_MESSAGE(LOG_ERR,"%s exists but PID %d is not running, will delete the file\n",fileName,theNaughtyPid);
	retVal=unlink(fileName);
	if(retVal<0) {
	  ARA_LOG_MESSAGE(LOG_ERR,"Error trying to unlink %s -- %s",fileName,strerror(errno));
	}
	return 0;
      }
    }
...
int writePidFile(const char *fileName)
{
  ...
  if(!(fpPid=fopen(fileName, "w"))) {
    ARA_LOG_MESSAGE(LOG_ERR,"Failed to open a file for PID, %s\n", fileName);
    return 1;
  }
  fprintf(fpPid,"%d\n", getpid()) ;
  fclose(fpPid) ;
...
uint32_t getRunNumber(char *baseDir) {
  int retVal=0;
  ...
  FILE *pFile;    
  pFile = fopen (runFileName, "r");
  if(!pFile) {
    ARA_LOG_MESSAGE (LOG_ERR,"Couldn't open %s\n",runFileName);
    return -1;
  }

  retVal=fscanf(pFile,"%u",&runNumber);
  if(retVal<0) {
    ARA_LOG_MESSAGE (LOG_ERR,"fscanff: %s ---  %s\n",strerror(errno),
            runFileName);
  }
  fclose (pFile);
  return runNumber;
