...
static int readBlock (void)
{
   ...
   while (wantBlock) {
      ...
      if (tPtr == NULL)  {
         if (feof (config)) {
            ARA_LOG_MESSAGE(LOG_ERR,"readBlock: End of file during read") ;
            retCode = CONFIG_E_EOF ;
         }
         else {
            ARA_LOG_MESSAGE(LOG_ERR,"readBlock: Error %d reading config", errno) ;
      ...
      if (*tPtr == '<') {
         if (tPtr[1] == '/') {
            ...
            ARA_LOG_MESSAGE(LOG_ERR,"Syntax error, nested blocks not supported") ;
...
/*               errMessage = kvpErrorString (kvpError()) ;*/
               ARA_LOG_MESSAGE(LOG_ERR,"configLoad: kvpParse error: %s, parsing <%s>",
                            ...
                            )
{
  //  fprintf(stderr,"Here too\n");
   ...
   ARA_LOG_MESSAGE(LOG_DEBUG,"configLoad Reading params from: %s\n", fileSpec) ;
   //   printf("confaaigLoad Reading params from: %s\n", fileSpec) ;
   ...
   if (config != NULL) {
      ...
      do {
         ...
         while (!wantBlock && (status == 0)) {
            ...
            if (status == CONFIG_E_OK) {
               ...
               for (block = 0 ; block < nBlocks ; block++) {
                  if (!foundBlock[block]) {
                     ARA_LOG_MESSAGE(LOG_WARNING,"<%s> block not found", blockName[block]) ;
                  }
               }
            }
         }
      ...
      if (errno == ENOENT) {
         ARA_LOG_MESSAGE(LOG_ERR,"configLoad: %s not found", fileSpec) ;
         status = CONFIG_E_NOFILE ;
      }
      else {
         ARA_LOG_MESSAGE(LOG_ERR,"configLoad: error %d opening %s", errno, fileSpec) ;
...
//   fileSpec = configFileSpec (fileName) ;
   ARA_LOG_MESSAGE(LOG_DEBUG,"configLoad Reading params from: %s", fileName) ;
   ...
   if (config != NULL) {
      ...
      if (errno == ENOENT) {
         ARA_LOG_MESSAGE(LOG_ERR,"configLoad: %s not found", fileName) ;
         status = CONFIG_E_NOFILE ;
      }
      else {
         ARA_LOG_MESSAGE(LOG_ERR,"configLoad: error %d opening %s", errno, fileName) ;
         status = CONFIG_E_SYSTEM ;
      }
   }
...
ConfigErrorCode readBlocks(char *fileName,char blockList[MAX_BLOCKS][BLOCKNAME_MAX],int *numBlocks)
{
   ...
   if (config != NULL) {
      ...
      if (errno == ENOENT) {
         ARA_LOG_MESSAGE(LOG_ERR,"readBlocks: %s not found", fileSpec) ;
         status = CONFIG_E_NOFILE ;
      }
      else {
         ARA_LOG_MESSAGE(LOG_ERR,"readBlocks: error %d opening %s", errno, fileSpec) ;
         status = CONFIG_E_SYSTEM ;
      }
   }
   *numBlocks=blockNum;
