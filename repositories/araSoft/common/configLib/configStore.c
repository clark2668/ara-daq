...
/* forward declarations */
 ...
 { 
     ...
     if(!fin) {
	 if(errorCounter<100) {
	     ARA_LOG_MESSAGE(LOG_ERR,"Error reading file %s -- Error %s\n",
     ...
     if(!fout) { 
 	if(errorCounter<100) { 
 	    ARA_LOG_MESSAGE(LOG_ERR,"Couldn't open %s for copy of %s -- Error %s\n", 
...
//    int retVal=0;
   ...
   if(stat (lastFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configSwitch couldn't find: %s",lastFileSpec);
       return CONFIG_E_NOFILE;
   }
   if(stat (oldFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configSwitch couldn't find: %s",oldFileSpec);
       return CONFIG_E_NOFILE;
   }
   ...
   ARA_LOG_MESSAGE(LOG_INFO,"configSwitch arcvhived: %s", archiveFileSpec);   
   return CONFIG_E_OK;
...
//    int retVal=0;
   ...
   if(stat (newFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configSwitch couldn't find: %s",newFileSpec);
       return CONFIG_E_NOFILE;
   }
   if(stat (oldFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configSwitch couldn't find: %s",oldFileSpec);
...
//   char mvCommand[2*FILENAME_MAX];
   ...
   if(stat (newFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configReplace couldn't find: %s",newFileSpec);
       return CONFIG_E_NOFILE;
   }
   if(stat (oldFileSpec, &fileStat) != 0) {
       ARA_LOG_MESSAGE(LOG_ERR,"configReplace couldn't find: %s",oldFileSpec);
       return CONFIG_E_NOFILE;
   }
...
/*    if(retVal<0) { */
/*        ARA_LOG_MESSAGE(LOG_ERR,"Problem storing config: %s %s ",archiveFileSpec,strerror(errno)); */
/*        return CONFIG_E_SYSTEM; */
/*    } */
/*    sprintf(mvCommand,"mv %s %s\n",oldFileSpec,archiveFileSpec); */
/*    retVal=system(mvCommand); */
/*    if(retVal<0) { */
/*        ARA_LOG_MESSAGE(LOG_ERR,"Problem archiving config: %s %s ",archiveFileSpec,strerror(errno)); */
/*        return CONFIG_E_SYSTEM; */
/*    } */
/*    sprintf(mvCommand,"mv %s %s\n",newFileSpec,oldFileSpec); */
/*    retVal=system(mvCommand); */
/*    if(retVal<0) { */
/*        ARA_LOG_MESSAGE(LOG_ERR,"Problem replacing config: %s %s ",archiveFileSpec,strerror(errno)); */
/*        return CONFIG_E_SYSTEM; */
/*    } */
   ARA_LOG_MESSAGE(LOG_INFO,"configReplace arcvhived: %s", archiveFileSpec);   
   return CONFIG_E_OK;
}
                             ...
                             )
{
   ...
   ARA_LOG_MESSAGE(LOG_INFO,"storeConfig writing params to: %s", fileSpec) ;

   config = creat (fileSpec, 0664) ;
   if (config == -1) {
      ARA_LOG_MESSAGE (LOG_ERR,"storeConfig: error %d creating %s", errno, fileSpec) ;
      return (CONFIG_E_SYSTEM) ;
   }
   else {
	  ...
	  /*errMessage = kvpErrorString (kvpError()) ;*/
         ARA_LOG_MESSAGE (LOG_ERR,"storeConfig: kvpWrite failed %s",kvpErrorString (kvpError()) ) ;
	 close(config);
         return (CONFIG_E_KVP) ;
      }
                             ...
                             )
{
   ...
   strcat (fileSpec, "/") ;
   strcat (fileSpec, fileName) ;
   ARA_LOG_MESSAGE(LOG_INFO,"configAppend writing params to: %s", fileSpec) ;

   config = open (fileSpec,O_CREAT|O_WRONLY|O_APPEND,0664) ;
   if (config == -1) {
      ARA_LOG_MESSAGE (LOG_ERR,"configAppend: error %d creating %s", errno, fileSpec) ;
      return (CONFIG_E_SYSTEM) ;
   }
   else {
      ...
      if (status == -1) {
	  /*errMessage = kvpErrorString (kvpError()) ;*/
         ARA_LOG_MESSAGE (LOG_ERR,"configAppend: kvpWrite failed %s",kvpErrorString (kvpError()) ) ;
	 close(config);
         return (CONFIG_E_KVP) ;
      }
      snprintf (tag, BLOCKNAME_MAX+3, "\n</%s>\n", blockName) ;
...
//	    ARA_LOG_MESSAGE(LOG_WARNING,"%s\t%s\n",blockName,blockList[blockNum]);
		...
		status = configAppend(tempFile,blockList[blockNum]);
	    if(status != CONFIG_E_OK) {
	      ARA_LOG_MESSAGE(LOG_WARNING,"Bugger -- configModifyIntArray\n");
	    }
	}
    }
    return configReplace(fileName,tempFile,rawTimePtr);
...
//	    ARA_LOG_MESSAGE(LOG_WARNING,"%s\t%s\n",blockName,blockList[blockNum]);
		...
		status = configAppend(tempFile,blockList[blockNum]);
	    if(status != CONFIG_E_OK) {
	      ARA_LOG_MESSAGE(LOG_WARNING,"Bugger -- configModifyFloatArray\n");
	    }
	}
    }
    return configReplace(fileName,tempFile,rawTimePtr);
...
//	    printf("%s\t%s\n",blockName,blockList[blockNum]);
		...
		status = configAppend(tempFile,blockList[blockNum]);
	    if(status != CONFIG_E_OK) {	       
	      ARA_LOG_MESSAGE(LOG_WARNING,"Bugger -- configModifyFloat\n");
	    }
	}
    }
    return configReplace(fileName,tempFile,rawTimePtr);
...
//	    printf("%s\t%s\n",blockName,blockList[blockNum]);
		...
		status = configAppend(tempFile,blockList[blockNum]);
	    if(status != CONFIG_E_OK) {
	      ARA_LOG_MESSAGE(LOG_WARNING,"Bugger -- configModifyString\n");
	    }
	}
    }
    return configReplace(fileName,tempFile,rawTimePtr);
